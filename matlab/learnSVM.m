...
names = dir('data/myNegatives/*.jpg') ;
...
names = dir('data/Original/*.png') ;


 [~, reindex] = sort( str2double( regexp( {names.name}, '\d+', 'match', 'once' )))
 names = names(reindex) ;
 
 names = fullfile('data', 'Original', {names.name}) ;
 
 labeledRect = gTruthEinstichspunkt.LabelData.(1);
for i=1:numel(names)
  im = imread(names{i}) ;
  disp(names{i});
  
  %disp(labeledRect{i});
  im = imcrop(im,labeledRect{i});
  %imshow(im);
  im = imresize(im, [64 64]) ;
  filename = sprintf('./data/cropped/%i.png', i);
  %imwrite(im, filename);
...
% save svm
%----------------
save('trainedSVM2','w','hogCellSize', 'scales');
